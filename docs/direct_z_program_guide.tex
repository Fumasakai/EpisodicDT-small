...
{\small 対象形式：\texttt{direct\_z\_conv\_episode\_v3} \quad 作成日：2026年9月18日}
\end{center}
...
ノイズ付き系列を$[B,1,L]$へ変換し、位置座標（$-1$から$1$）を追加する。
1$\times$1畳み込みで64チャネルへ変換し、4個の時間方向残差ブロックへ渡す。
各ブロックはカーネル幅3の畳み込みを2回用い、dilationを順に1、2、4、8とする。
$z$と拡散ステップの埋込みを各ブロックにスケール・シフトとして与える。
GroupNorm・SiLU・1$\times$1畳み込みで$[B,L,1]$の$v$を出力する。
畳み込み経路の受容野は61ステップであり、パディングによって系列長を保つ。
全体生成用なので因果マスクは用いない。Encoderは引き続き同時学習する。
旧MLP方式は比較用設定\texttt{configs/mlp\_baseline.yaml}から利用できる。
...
\subsection{図と出力ファイル}
図と指標の保存先は\path{results/figures/direct_z_conv_episode_v3/}である。
...
ひげを最大・最小値と解釈しない。全値の箱ひげ図では生成側は元データの$S$倍の標本数を持つ。
系列比較図では緑の折れ線が元系列、青の各折れ線が個々の生成系列である。
元エピソード1本を無作為に選び、対応する生成系列から重複なしで8本を選ぶ。
各パネルに同じ元系列と生成系列1本を表示する。選択は評価の乱数seedで再現できる。
生成本数が8本未満の場合は利用可能な全本数を表示する。
評価JSONのMAEは生成平均と元系列との誤差である。
